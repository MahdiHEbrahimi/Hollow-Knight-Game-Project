...
public class GuidePanel extends BasePanel {
    ...
    public GuidePanel() {
        ...
        // --- عنوان ---
        Label title = new Label("GUIDE", new Label.LabelStyle(font, font.getColor()));
        title.setFontScale(2.5f);
        add(title).padBottom(30f).row();

        // --- جدول محتوا (داخل اسکرول) ---
        Table content = new Table();
        content.align(Align.center);    // ← کل جدول وسط‌چین شود
        content.pad(30f);
        content.defaults().padBottom(10f);

        // ═══════════ کنترل‌ها ═══════════
        Label controlsHeader = new Label("--- CONTROLS ---", new Label.LabelStyle(font, font.getColor()));
        ...
        for (GameAction action : GameAction.values()) {
            ...
            Label lbl = new Label(line, new Label.LabelStyle(font, font.getColor()));
            lbl.setAlignment(Align.center);
            lbl.setWrap(false);   // هر کنترل در یک خط
            content.add(lbl).center().padBottom(5f).row();
        }

        content.row().padTop(25f);

        // ═══════════ توانایی‌ها ═══════════
        Label abilitiesHeader = new Label("--- ABILITIES ---", new Label.LabelStyle(font, font.getColor()));
        ...
        for (String ab : abilities) {
            Label lbl = new Label("  " + ab, new Label.LabelStyle(font, font.getColor()));
            lbl.setWrap(true);
            lbl.setAlignment(Align.center);
            content.add(lbl).width(700f).center().padBottom(6f).row();
        }

        content.row().padTop(25f);

        // ═══════════ کدهای تقلب ═══════════
        Label cheatsHeader = new Label("--- CHEAT CODES ---", new Label.LabelStyle(font, font.getColor()));
        ...
        for (CheatCode cheat : CheatCode.values()) {
            ...
            Label lbl = new Label(text, new Label.LabelStyle(font, font.getColor()));
            lbl.setWrap(true);
            lbl.setAlignment(Align.center);
            content.add(lbl).width(700f).center().padBottom(6f).row();
        }
        ...
        add(scroll).expand().fill().center().padBottom(20f).row();   // ScrollPane در وسط
